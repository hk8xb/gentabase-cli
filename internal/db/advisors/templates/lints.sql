...
where
    ...
    and fk.schema_name not in (
        '_timescaledb_cache', '_timescaledb_catalog', '_timescaledb_config', '_timescaledb_internal', 'auth', 'cron', 'extensions', 'graphql', 'graphql_public', 'information_schema', 'net', 'pgmq', 'pgroonga', 'pgsodium', 'pgsodium_masks', 'pgtle', 'pgbouncer', 'pg_catalog', 'pgtle', 'realtime', 'repack', 'storage', 'gentabase_functions', 'gentabase_migrations', 'tiger', 'topology', 'vault'
    )
...
where
    ...
    and schema_name not in (
        '_timescaledb_cache', '_timescaledb_catalog', '_timescaledb_config', '_timescaledb_internal', 'auth', 'cron', 'extensions', 'graphql', 'graphql_public', 'information_schema', 'net', 'pgmq', 'pgroonga', 'pgsodium', 'pgsodium_masks', 'pgtle', 'pgbouncer', 'pg_catalog', 'pgtle', 'repack', 'storage', 'gentabase_functions', 'gentabase_migrations', 'tiger', 'topology', 'vault'
    )
    and (
        -- Example: auth.uid()
        (
...
where
    ...
    and r.rolname not like 'gentabase%admin'
...
select
    ...
    'EXTERNAL' as facing,
    array['SECURITY'] as categories,
    'Detects when Gentabase Auth user_metadata is referenced insecurely in a row level security (RLS) policy.' as description,
    format(
        'Table `%s.%s` has a row level security policy `%s` that references Gentabase Auth `user_metadata`. `user_metadata` is editable by end users and should never be used in a security context.',
        schema_name,
        table_name,
        policy_name
    ) as detail,
