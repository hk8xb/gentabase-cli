...
Initialize configurations for Gentabase local development.
...
In addition to `config.toml`, the `supabase` directory may also contain other Gentabase objects, such as `migrations`, `functions`, `tests`, etc.
